...
Usage::
    ...
    python train.py configs/xlm_roberta_base.yaml optimizer.lr=3e-5
    tc-train configs/xlm_roberta_large.yaml training.batch_size=8
...
from text_classification.data.dataset import TextClassificationDataModule
from text_classification.training.lightning_module import TextClassificationModule
from text_classification.utils.config import load_config, validate_config
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Train a multi-label text classifier.")
    ...
    # Resolve tasks and class definitions from config
    ...
    # Model
    model_module = TextClassificationModule(
        cfg=cfg,
        tasks=tasks,
        class_lists=class_lists,
    ...
    logger.info("Starting training (max_steps=%d)…", cfg.training.max_steps)
